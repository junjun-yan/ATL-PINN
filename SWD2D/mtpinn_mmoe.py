...
task_number = 2
expert_layers = [3] + 3 * [100]
tower_layers = 2 * [100] + [3]

data_path = r'/root/ATLPINN/data/SWD.h5'
result_path = r'/root/ATLPINN/result/swd2d_mmoe.txt'
...
def run(tasks):
    ...
    log_path = "/root/ATLPINN/log/SWD2D-mmoe-%s-%s.log" % (tasks[0], create_date)
    ...
    network = NeuralNetwork_Soft_MMOE(X, expert_layers, tower_layers, task_number, device)
    ...
    for task in range(task_number):
        ...
        # save prediction
        mmoe_path = "/root/ATLPINN/output/swd_mmoe_%d.npy" % (tasks[task])
        mmoe_data = {'h': h_pred[task]}
        np.save(mmoe_path, mmoe_data)


if __name__ == '__main__':
    task = [str(0).rjust(4, '0'), str(1).rjust(4, '0')]
    run(task)
