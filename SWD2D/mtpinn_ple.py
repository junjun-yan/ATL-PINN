...
task_number = 2
shared_layers = [3] + 3 * [100]
expert_layers = [3] + 2 * [100]
tower_layers = 2 * [100] + [3]

data_path = r'/root/ATLPINN/data/SWD.h5'
result_path = r'/root/ATLPINN/result/swd2d_ple.txt'
...
def run(tasks):
    ...
    log_path = "/root/ATLPINN/log/SWD2D-ple-%s-%s.log" % (tasks[0], create_date)
    ...
    for task in range(task_number):
        ...
        # save prediction
        ple_path = "/root/ATLPINN/output/swd_ple_%d.npy" % (tasks[task])
        ple_data = {'h': h_pred[task]}
