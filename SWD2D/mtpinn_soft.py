...
expert_layers = [3] + 5 * [100]
tower_layers = 2 * [100] + [3]

data_path = r'/root/ATLPINN/data/SWD.h5'
result_path = r'/root/ATLPINN/result/swd2d_soft.txt'
...
def run(tasks):
    ...
    log_path = "/root/ATLPINN/log/SWD2D-soft-%s-%s.log" % (tasks[0], create_date)
    ...
    for task in range(task_number):
        ...
        # save prediction
        soft_path = "/root/ATLPINN/output/swd_soft_%d.npy" % (tasks[task])
        soft_data = {'h': h_pred[task]}
        np.save(soft_path, soft_data)

if __name__ == '__main__':
    for i in range(64, 100, 2):
