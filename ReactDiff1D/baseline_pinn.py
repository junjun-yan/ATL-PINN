...
from pinns import PINN_ReactDiff1D
...
result_path = r'/root/ATLPINN/result/diffreact_pinn.txt'
...
def run(tasks):
    ...
    log_path = "/root/ATLPINN/log/ReacDiff-PINN-%d-%s.log" % (tasks[0], create_date)
    pinn_path = "/root/ATLPINN/output/diffreact_pinn_%d.npy" % (tasks[0])
    solver_path = '/root/ATLPINN/output/diffreact_solver_%d.npy' % (tasks[0])
    ...
    model = PINN_ReactDiff1D(X_init, Y_init, X_l_bound, X_r_bound, X_test, Y_test, X_eqns, network,
