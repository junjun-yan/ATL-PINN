...
result_path = r'/root/ATLPINN/result/diffreact_mmoe.txt'


def run(tasks):
    create_date = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime(time.time()))
    log_path = "/root/ATLPINN/log/ReacDiff-MMOE-%d-%d-%s.log" % (tasks[0], tasks[1], create_date)
    ...
    for task in range(task_number):
        ...
        # save prediction
        mmoe_path = "/root/ATLPINN/output/diffreact_mmoe_%d.npy" % (tasks[task])
