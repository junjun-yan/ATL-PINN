...
print_path = r'/root/MTPINN/log4loss/burgers_mmoe_1.txt'


def run(tasks):
    create_date = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime(time.time()))
    log_path = "/root/tf-logs/Burgers1D-MTPINN-mmoe-%d-%d-%s.log4loss" % (tasks[0], tasks[1], create_date)
    ...
    for task in range(task_number):
        ...
        # save prediction
        mmoe_path = "/root/MTPINN/output/burgers_mmoe_-1_%d.npy" % (tasks[task])
        mmoe_data = {'u': Y_pred[task]}
        np.save(mmoe_path, mmoe_data)


with open('/root/MTPINN/Burgers1D/index.txt', 'r') as f:
