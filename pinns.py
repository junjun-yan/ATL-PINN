...
class PINN_Advection1D(PINN):
    def __init__(self, X_init, Y_init, X_l_bound, X_r_bound, X_test, Y_test, X_eqns, network, batch_size, beta,
                 init_weight, bound_weight, log_path, learning_rate, device):
        super().__init__(network, X_test, Y_test, batch_size, init_weight, bound_weight, log_path,
                         learning_rate, device)
...
class PINN_DiffSorb1D(PINN):
    ...
    def loss_pde(self):
        # pde loss function: Diffusion sorption 1D
        D: float = 5e-4
        por: float = 0.29
        rho_s: float = 2880
        k_f: float = 3.5e-4
    ...
    def loss_bc(self):
        ...
        # left is 1 and right is second boundary condition
        D: float = 5e-4
        ...
        # right -> D*du/dx
        u = Y_r_pred[:, 0:1]
        u_g = gradients(u, self.X_r_bound)[0]
        u_x = u_g[:, 0:1]
        Y_r = D * u_x

        return mean_squared_error(Y_l_pred, Y_l) + mean_squared_error(Y_r_pred, Y_r)

    def loss_ic(self):
        # initial condition loss function: points
        Y_pred = self.network(self.X_init)
        return mean_squared_error(Y_pred, self.Y_init)
